...
public class RepositorioDeEmpresas {
	...
	public static void agregarEmpresas(Empresa empresaAAgregar){
		...
		String nombre = empresaAAgregar.toString();
		Empresa empresa = em.createQuery("from Empresa e where e.nombre = nombre", Empresa.class) //RECONTRA DUDOSO
        .getResultList().get(0);
	 ...
	 //entityManager().createQuery("from Consultora c where c.nombre like :nombre", Consultora.class).setParameter("nombre", "%" + nombre + "%").getResultList();
